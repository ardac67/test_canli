...
io.on('connection', (socket) => {
  ...
  socket.on('join_room', (data) => {
    ...
    console.log(`user:${socket.id} user_id: ${data.user},user_name: ${data.user_name}, user_room: ${data.room}`);
  });
  socket.on('send_message', (data) => {
    //{ user: user, message: message, room: 'room1' }
    io.to(data.room).emit('receive_message', data);
    console.log(`user:${socket.id} user_name: ${data.user}, message: ${data.message}`, data.room);
  })
});
